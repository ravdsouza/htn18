...
class ClarifaiHandler{
	...
	triggerPredict(arr) {
		...
		for(var i = 0; i < arr.length; i++) {
			var url = arr[i].input.data.image.url;
			this.app.models.predict(Clarifai.FOOD_MODEL, url).then(
				function(response) {
					console.log(response);
					var concepts = response.outputs[0].data.concepts;
					var j = 0;
					while(j < 3) {
						masterArr[j]=concepts[j];
						j=j+1;
						console.log(concepts[0]);
					}
					break;
					// masterArr[i] = concepts;
...
const clarifai = new ClarifaiHandler();
clarifai.generateFakeInfo();
clarifai.searchFood();
